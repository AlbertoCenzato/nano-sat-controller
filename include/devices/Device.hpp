 ...
 *		  The device can perform self-tests diagnosing its status. If a subclass needs
 *		  to add specific tests to be executed it should add a TestFunction and an
 *		  ActionFunction calling addTestAndAction(test, action) in its constructor.
...
public:
	...
	explicit Device(const std::string &deviceID = DEFAULT_DEV_ID);
	virtual ~Device();
	...
	/**
	 * @brief This virtual function must be override by subclasses.
...
protected:
	 ...
	 *		  
	 * @note It receives two Device member functions of type TestFunction and ActionFunction.
