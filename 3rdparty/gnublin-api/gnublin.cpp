...
gnublin_smtp::gnublin_smtp()
{
	...
	if((RecvBuf = new char[GNUBLIN_BUFFER_SIZE]) == NULL)
		throw gnublin_smtp(gnublin_smtp::LACK_OF_MEMORY);
	
	if((SendBuf = new char[GNUBLIN_BUFFER_SIZE]) == NULL)
		throw gnublin_smtp(gnublin_smtp::LACK_OF_MEMORY);
}
...
void gnublin_smtp::Send()
{
	...
	for(FileId=0;FileId<Attachments.size();FileId++)
	{
		...
		// sending the file:
		if(TotalSize/1024 > GNUBLIN_MSG_SIZE_IN_MB*1024)
		...
		else
		{
			...
			for(i=0;i<FileSize/54+1;i++)
			{
				...
				strcat(SendBuf,"\r\n");
				MsgPart += res + 2;
				if(MsgPart >= GNUBLIN_BUFFER_SIZE/2)
...
SOCKET gnublin_smtp::ConnectRemoteServer(const char *szServer,const unsigned short nPort_)
{
	...
	timeout.tv_sec = GNUBLIN_TIME_IN_SEC;
	timeout.tv_usec = 0;
...
void gnublin_smtp::ReceiveData()
{
	...
	time.tv_sec = GNUBLIN_TIME_IN_SEC;
	time.tv_usec = 0;
