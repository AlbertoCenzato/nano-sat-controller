...
AlignmentSensor::AlignmentSensor(const utils::AlignmentSensorSettings& settings) 
	...
	// in a unique_ptr because there's no guarantee about when a is a static 
	// object initialized. The standard says "before usage", ok but when?
	// Better a lazy contructor than an unnecessarly blocked camera?
	if (videoCap == nullptr) {
...
void AlignmentSensor::readCamera() {
	...
	cv::Mat lines;
	cv::HoughLines(picture, lines, 1, M_PI / 180, 200); // FIXME: magic numbers!
